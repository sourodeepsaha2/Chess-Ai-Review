        ...
        {status === 'success' && response && (
          <>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/30 p-3.5 backdrop-blur-xl transition-all duration-300">
              ...
              >
                ...
                {!showSummary && response.summary && (
                  <span className="text-[10px] text-slate-400 font-mono font-semibold max-w-[180px] truncate text-right">
                    {response.summary.openingName || response.game?.opening?.name} | W: {response.summary.whiteAccuracy}% B: {response.summary.blackAccuracy}%
                  </span>
                )}
              </div>
              ...
              {showSummary && (
                <div className="mt-3">
                  <SummaryCard response={response} onSelectMove={setSelectedIndex} />
                </div>
              )}
            </div>
